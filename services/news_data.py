...
def get_udn_news(stock_id, page_limit=1) -> pd.DataFrame:
    ...
    while 1:
        if page > page_limit:
            print(f"已達設定的頁數上限：{page}{' '*50}", end="\r")
            break
        url = f"https://udn.com/api/more?page={page}&id=search:{stock_name}&channelId=2&type=searchword&last_page=100"
        ...
        for i in range(len(json_news)):
            ...
            try:
                ...
                if time.mktime(time.gmtime())-31536000>news_time.timestamp(): break  # 一年內
...
def stock_news_split_word(stock_id: str):
    ...
    threshold = max(4, len(filtered_words) // 800)
